...
PMatrix PMatrix::rotaryX(double dtheta){
    ...
    Rx.entry(1,0) = 0;
    Rx.entry(1,1) = cos(dtheta);
    Rx.entry(1,2) = -sin(dtheta);

    Rx.entry(2,0) = 0;
    Rx.entry(2,1) = sin(dtheta);
    Rx.entry(2,2) = cos(dtheta);
...
PMatrix PMatrix::rotaryY(double dtheta){
    ...
    Ry.entry(2,0) = -sin(dtheta);
    Ry.entry(2,1) = 0;
    Ry.entry(2,2) = cos(dtheta);
...
PMatrix PMatrix::rotaryZ(double dtheta){
    ...
    Rz.entry(1,0) = sin(dtheta);
    Rz.entry(1,1) = cos(dtheta);
    Rz.entry(1,2) = 0;
...
PMatrix PMatrix::rotaryHX(double dtheta, vector3d d){
    PMatrix Rx(4);
...
PMatrix PMatrix::rotaryHY(double dtheta, vector3d d){
    PMatrix Ry(4);
...
PMatrix PMatrix::rotaryHZ(double dtheta, vector3d d){
    PMatrix Rz(4);
